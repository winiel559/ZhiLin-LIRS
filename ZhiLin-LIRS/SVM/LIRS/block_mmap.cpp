...
struct problem my_get_block(BlockProblem *bprob, int sub_id, unsigned long long *offset, int **assign_table, FILE *fp, int *num_feature, uint8_t *training_data_mmap)
{
	struct problem subprob;
	...
	for(int i=0; i<subprob.l; i++)
	{
		...
		pt_label = (int *)training_data_mmap + offset[instance_id]/4;
		...
		memcpy(&subprob.y[i], pt_label, sizeof(int)*1);
		...
		for(int j=0; j<num_feature[instance_id]; j++)
		{
			...
			memcpy(&xi->index, pt_index, sizeof(int)*1);
			pt_index += 1;
			xi++;
			temp++;
		}
		...
		pt_value = (double*) pt_index;
	
		for(int j=0; j<num_feature[instance_id]; j++)
		{
			//fread(&xi->value, sizeof(double), 1, fp);
			memcpy(&xi->value, pt_value, sizeof(double)*1);
			pt_value += 1;
			xi++;
		}
...
//----------initial load--------------
	...
	fd = open(training_binary_dir, O_RDONLY);
	
	fstat(fd, &sb);
	training_data_mmap = static_cast<uint8_t*>(mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0)); 
	
	if (-1==madvise(training_data_mmap, sb.st_size, MADV_SEQUENTIAL))
    {
	...
	memcpy(&num_feature[0], training_data_mmap, sizeof(int)*l);
...
while (iter < max_iter) {
		...
		for (int i=0; i < nBlocks; i++) {
			...
			//struct problem *subprob = bprob->get_block(perm[i]);
			startload_t = time(NULL);
			struct problem subprob = my_get_block(bprob, i, byte_offset, assign_table, training_data, num_feature, training_data_mmap);
