...
void Solver_MCSVM_CS::Solve(double *w, double *alpha, double *_stopping, bool *solved=NULL)
{
	int i, m, s;
	int iter = 0;
	...
	double *_PGmax, double *_PGmin, int max_iter=2000, bool *solved=NULL)
{
	...
	if(solver_type == L2R_L1LOSS_SVC_DUAL|| solver_type == L2R_L1LOSS_SVC_DUAL_SUBSAMPLE || solver_type == L2R_L1LOSS_SVC_DUAL_AVG)
	{
		diag[0] = 0;
		diag[2] = 0;
		upper_bound[0] = Cn;
...
void pegasos_solve_subprob(const problem *prob, int n, double *a, double *w, double *wnorm, double lambda, int t)
{
	int i;
	int l = prob->l;  	// |A_t|
	if(l == 0) 
	...
	for(int s=0;s<l;s++) 
	{
		...
		// Scale W by (1-eta*lambda)
		double eta = 1.0/(lambda*(t+s+2));
		//double eta = 1/(lambda*(s+2));

		(*a) = (*a) * (1-eta*lambda);
		(*wnorm) = (*wnorm) * (1-eta*lambda)*(1-eta*lambda);
		...
		{ 
			...
			{
				int idx = xi->index - 1;
				double addterm = eta*prob->y[i]*(xi->value);
				*wnorm = (*wnorm) + 2.0*addterm*(*a)*w[idx] + (addterm*addterm);
				addterm /= (*a);
				w[xi->index-1] += addterm;
				xi++;
			}
		}
		...
		if((*wnorm) > 1.0/lambda) 
		{
			double scale = sqrt(1.0/(lambda*(*wnorm)));
			*a = (*a) * scale;
			(*wnorm) *= (scale * scale);
			//for(i=0;i<n;i++) w[i] *= scale;
		}
...
void pegasos_update_subgradient(const problem *prob, int n, double *w, double lambda, double eta)
{
	int i;
	int l = prob->l;  	// |A_t|
	int *I = new int[l]; //misclassified examples
	int sizeI = 0;
	...
	// Scale W by (1-eta*lambda)
	for(i=0;i<n;i++) w[i] *= (1.0 - eta*lambda);

	// and add sub-gradients
	for(i=0;i<sizeI;i++) 
	{
		feature_node *xi = prob->x[I[i]];
		while(xi->index!=-1)
		{
			w[xi->index-1] += (eta/l)*prob->y[I[i]]*(xi->value);
			xi++;
		}
	}
	double wnorm2 = 0;
	for(i=0; i<n;i++) wnorm2 += w[i]*w[i];

	// Project if needed
	if(wnorm2 > 1.0/lambda) 
	{
		double scale = sqrt(1.0/(lambda*wnorm2));
		for(i=0;i<n;i++) w[i] *= scale;
	}
...
model* train(const problem *prob, const parameter *param)
{
	...
	{
		double *alpha=Malloc(double, l*nr_class);
		double stopping;
		...
		Solver.Solve(model_->w, alpha, &stopping);
		free(alpha);
	}
	else
	{
...
{
	"L2R_LR", "L2R_L2LOSS_SVC_DUAL", "L2R_L2LOSS_SVC","L2R_L1LOSS_SVC_DUAL","MCSVM_CS", "L1R_L2LOSS_SVC","L1R_LR", "L2R_L1LOSS_SVC_PEGASOS", "L2R_L1LOSS_SVC_PEGASOS_INNER", NULL
};
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_LR
		...
		&& param->solver_type != L2R_L1LOSS_SVC_PEGASOS_INNER
		&& param->solver_type != L2R_L2LOSS_SVC_DUAL_AVG
		&& param->solver_type != L2R_L1LOSS_SVC_DUAL_AVG
		&& param->solver_type != L2R_L1LOSS_SVC_DUAL_SUBSAMPLE
		)
