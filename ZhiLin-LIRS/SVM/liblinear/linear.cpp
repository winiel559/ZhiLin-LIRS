...
double evaluate_testing(double *w, int n, const problem *prob_t, int positive_label =1)
{
	...
	for(int l =0; l < prob_t->l; l++){
		int idx;
		ty[l] = prob_t->y[l] == positive_label ? 1 : -1;
		dec_value[l]=0;
		indices[l] = l;
		for( const feature_node *lx=tx[l]; (idx=lx->index)!=-1; lx++){
			if(idx<= n)
				dec_value[l] += w[idx-1]*lx->value;
		}

		if (dec_value[l] > 0 && ty[l] == 1)
			++correct;
		else if (dec_value[l] <=0 && ty[l] == -1)
			++correct;
	}
	printf("accuracy %lf ", (double)correct/ prob_t->l);
	std::sort(indices, indices+prob_t->l, Comp(dec_value));
	int tp = 0,fp = 0;
	double roc = 0;
...
// For L2R_L1LOSS_SVC_DUAL and L2R_L2LOSS_SVC_DUAL 
double primal_value(const problem *prob, int solver_type, double C,const double *w) 
{
	int l = prob->l;
	int w_size = prob->n;
	int i;
...
static void solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, int solver_type, const parameter* param)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
	...
	cpu_time += (double) (clock() - start_clock_t) / CLOCKS_PER_SEC;
	...
	while (iter < max_iter)
	{
		...
		for (s=0;s<active_size;s++)
		{
			...
			if( 0 && (s+1) % param->blocksize == 0) {
				...
				info("iter %d.%d time %lf runtime %lf loadtime %lf cputime %lf obj %lf ",iter, s/param->blocksize+1,  total_time, total_time, total_time - cpu_time, cpu_time, v/2);
				if(param->showprimal) 
				{
					double primal = primal_value(prob, solver_type,  Cp, w);
					info("primal %lf ", primal);
				}
				if(param->prob_t!=NULL) 
					info("aoc %lf ", 100*evaluate_testing(w, w_size, param->prob_t, param->positive_label));
				info("\n");
			}
		}
		...
		if(param->equal_part <=0)
		{
			...
			info("iter %d time %lf runtime %lf loadtime %lf cputime %lf obj %lf ",iter,  total_time, total_time, total_time - cpu_time, cpu_time, v/2);
			if(param->showprimal) 
			{
				double primal = primal_value(prob, solver_type, Cp, w);
				info("primal %lf ", primal);
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL, param);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL, param);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
	...
	else
	{
		if(nr_class == 2)
		{
			...
			model_->param.positive_label = label[0];
			if(model_->param.equal_part <= 0)
				train_one(&sub_prob, &(model_->param), &model_->w[0], weighted_C[0], weighted_C[1]);
			else 
			{
				int l_real = sub_prob.l;
				int equal_part = model_->param.equal_part;
				for(int m=0; m < equal_part; m++) {
					sub_prob.l = min(l_real/equal_part*(m+1), l_real);
					train_one(&sub_prob, &(model_->param), &model_->w[0], weighted_C[0], weighted_C[1]);
					info("#inst %d ", sub_prob.l);
					if(param->prob_t!=NULL)
						info("aoc %lf ", 100*evaluate_testing(&model_->w[0], w_size, param->prob_t, model_->param.positive_label));
					info("\n");
				}
